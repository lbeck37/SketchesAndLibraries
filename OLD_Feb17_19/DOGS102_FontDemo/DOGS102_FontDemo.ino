...
boolean bScreenChanged() {
   ...
   switch (sCurrentMode) {
      case sNormalMode:
         bChanged= bNormalScreenChanged();
         break;
      case sCalibMode:
         bChanged= bCalibScreenChanged();
         break;
      default:
         Serial << "bScreenChanged(): Bad case in switch()= " << sCurrentMode << endl;
         break;
   }  //switch
   sClearScreenChangedFlags();
   return bChanged;
}  //bScreenChanged
boolean bNormalScreenChanged() {
   boolean bChanged= bGearChanged || bButtonsChanged || bServoChanged;
   return bChanged;
}  //bNormalScreenChanged


boolean bCalibScreenChanged() {
   boolean bChanged= bButtonsChanged || bServoChanged;
   return bChanged;
}  //bCalibScreenChanged


int sClearScreenChangedFlags() {
...
boolean bScreenChanged() {
   //Determine if something being displayed has changed
   boolean bChanged= bGearChanged || bButtonsChanged || bServoChanged;
   //Clear the flags.
   bGearChanged= bButtonsChanged= bServoChanged= false;
   return bChanged;
...
int sDisplayCurrentGear() {
   if (sCurrentMode == sNormalMode) {
      itoa(sCurrentGear, szValue, 10);
      Serial << "sDisplayCurrentGear(): Mode= " << sCurrentMode
             << ", szValue= " << szValue << endl;
      sDisplayText(0, 4, sFontBigNum, szValue);    //Space the first character in 4 pixels.
   }  //if (sCurrentMode..
